...
foreach ($questionsJson as $questionObject) {
    foreach($questionObject as $answers) {
        foreach($answers as $answer) {
            $answerArray[] = $answer;
        }
    }
    $questions = new Question ($questionObject->id, $questionObject->questionText, $answerArray);
    var_dump($questions);
    $answerArray = array();
}